...
LABEL org.opencontainers.image.description="Generic Dockerized Pearl (PRL) GPU miner with optional SSH-relay tunnel"
...
RUN sed -i 's@//.*archive.ubuntu.com@//mirrors.aliyun.com@g; s@//.*security.ubuntu.com@//mirrors.aliyun.com@g' /etc/apt/sources.list \
    && apt-get update && apt-get install -y --no-install-recommends \
        ca-certificates curl tini openssh-client autossh socat \
...
COPY entrypoint.sh health.sh /opt/miner/
